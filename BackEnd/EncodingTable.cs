...
namespace BackEnd
{
    public class EncodingTable
    {
        ...
        /// <summary>
        ///     The backing store of the encoding table, that contains code (or null if the byte has no code) for each byte from 0
        ///     to 255.
        /// </summary>
        private readonly int[][] _table = new int[CodesInTable][];
        ...
        public int[] this[int byteValue]
        {
            get
            {
                ...
                return _table[byteValue] ??
                       throw new ArgumentException(nameof(byteValue), "The provided byte has no code.");
            }
            private set
            {
                CheckIndexBounds(byteValue);
        ...
        {
            for (var @byte = MinTableIndex; @byte <= MaxTableIndex; @byte++) this[@byte] = null;
        }
            ...
            for (var @byte = MinTableIndex; @byte <= MaxTableIndex; @byte++)
                ...
                try
                {
                    ...
                    result += "\n" +
                              ...
                              + " - ";
                    foreach (var bit in nextByte) result += bit;
                }
