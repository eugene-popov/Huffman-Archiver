...
namespace BackEnd
{
    ...
    public class Encoder
    {
        ...
        public void Encode()
        {
            ...
            var currentTotal = 50; // current percentage (regarding compression process)
            var nextByte = InputStream.ReadByte();
            while (nextByte != -1)
            {
                var newTotal = (int) (50 + InputStream.Position * 1.0 / (totalBytes * 2) * 100);
                ...
                for (var bitIndex = 0; bitIndex < code.Length; bitIndex++)
        ...
        private EncodingTable EncodingTable
        {
            get => _encodingTable;
            set => _encodingTable =
                value ?? throw new ArgumentNullException(nameof(value), "The provided encoding table is null.");
        }
